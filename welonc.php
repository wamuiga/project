...
if (isset($_GET["action"]) && $_GET["action"]=="new_patient") {
    $result=queryMysql("SELECT id FROM oncdb WHERE exit_stamp IS NULL ORDER BY id DESC;");
    $row=$result->fetch_array(MYSQLI_ASSOC);
    $id=$row['id'];
    
    $result=queryMysql("UPDATE `oncdb` SET `exit_stamp` = CURRENT_TIMESTAMP WHERE `oncdb`.`id` = $id;");


}


$result=queryMysql("SELECT COUNT(id) as remaining FROM oncdb WHERE exit_stamp IS NULL;");
$row=$result->fetch_array(MYSQLI_ASSOC);
  ...
  if (isset($_POST["send"]))
{
  $username = trim($_SESSION["username"]); 
   ...
   $query = "INSERT INTO oncdrtp (  username ) values ('$username')";
...
<div class="navbar">
    ...
    <a href="logout.php" class="btn alt bg-dark">Sign Out of Your Account</a>
     </div>
...
<div class="grid-item">
        ...
        <a href="welonc.php?action=new_patient" > 
