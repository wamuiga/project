...
if (isset($_GET["action"]) && $_GET["action"]=="new_patient") {
    $result=queryMysql("SELECT id FROM opthdb WHERE exit_stamp IS NULL ORDER BY id DESC;");
    $row=$result->fetch_array(MYSQLI_ASSOC);
    $id=$row['id'];
    
    $result=queryMysql("UPDATE `opthdb` SET `exit_stamp` = CURRENT_TIMESTAMP WHERE `opthdb`.`id` = $id;");


}


$result=queryMysql("SELECT COUNT(id) as remaining FROM opthdb WHERE exit_stamp IS NULL;");
$row=$result->fetch_array(MYSQLI_ASSOC);
  ...
  if (isset($_POST["send"]))
{
  $username = trim($_SESSION["username"]); 
   ...
   $query = "INSERT INTO opthdrtp (  username ) values ('$username')";
...
<div class="grid-item">
        ...
        <a href="welopth.php?action=new_patient" > 
...
foreach($mysqli->query('SELECT SUM(total_tickets) FROM opthdb' ) as $row ){
    ...
    <div class='card-body p-0 clearfix'>
    <i class='fa fa-users bg-flat-color-6 p-3 font-2xl mr-3 float-left text-light'></i>
    <h5><br> Ophthalmology</h5><br><br><br>";
    echo "TOTAL tickets : " . $row['SUM(total_tickets)'];
