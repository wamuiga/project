...
if (isset($_GET["action"]) && $_GET["action"]=="new_patient") {
    $result=queryMysql("SELECT id FROM phydb WHERE exit_stamp IS NULL ORDER BY id DESC;");
    $row=$result->fetch_array(MYSQLI_ASSOC);
    $id=$row['id'];
    
    $result=queryMysql("UPDATE `phydb` SET `exit_stamp` = CURRENT_TIMESTAMP WHERE `phydb`.`id` = $id;");


}


$result=queryMysql("SELECT COUNT(id) as remaining FROM phydb WHERE exit_stamp IS NULL;");
$row=$result->fetch_array(MYSQLI_ASSOC);
  ...
  if (isset($_POST["send"]))
{
  $username = trim($_SESSION["username"]); 
   ...
   $query = "INSERT INTO phydrtp (  username ) values ('$username')";
...
<div class="grid-item">
        ...
        <a href="welphy.php?action=new_patient" > 
...
foreach($mysqli->query('SELECT SUM(total_tickets) FROM entdb' ) as $row ){
    ...
    <h5><br> Ear nose and throat (ENT)</h5>";
...
foreach($mysqli->query('SELECT SUM(total_tickets) FROM phydb' ) as $row ){
    ...
    <div class='col-md-4 col-lg-2'>
    <div class='card text-black bg-flat-color-6'><a href='welphy.php'>
    <div class='card-body p-0 clearfix'>
    <i class='fa fa-users bg-flat-color-6 p-3 font-2xl mr-3 float-left text-light'></i>
    <h5> Pharmacy</h5><br><br><br>";
    echo "TOTAL tickets : " . $row['SUM(total_tickets)'];
