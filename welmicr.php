...
if (isset($_GET["action"]) && $_GET["action"]=="new_patient") {
    $result=queryMysql("SELECT id FROM micrdb WHERE exit_stamp IS NULL ORDER BY id DESC;");
    $row=$result->fetch_array(MYSQLI_ASSOC);
    $id=$row['id'];
    
    $result=queryMysql("UPDATE `micrdb` SET `exit_stamp` = CURRENT_TIMESTAMP WHERE `micrdb`.`id` = $id;");


}


$result=queryMysql("SELECT COUNT(id) as remaining FROM micrdb WHERE exit_stamp IS NULL;");
$row=$result->fetch_array(MYSQLI_ASSOC);
  ...
  if (isset($_POST["send"]))
{
  $username = trim($_SESSION["username"]); 
   ...
   $query = "INSERT INTO micrdrtp (  username ) values ('$username')";
...
<div class="grid-item">
        ...
        <a href="welmicr.php?action=new_patient" > 
...
foreach($mysqli->query('SELECT SUM(total_tickets) FROM micrdb' ) as $row ){
    ...
    <div class='card-body p-0 clearfix'>
    <i class='fa fa-users bg-flat-color-6 p-3 font-2xl mr-3 float-left text-light'></i>
    <h5><br>Microbiology</h5><br><br><br></h5>";
    echo "TOTAL tickets : " . $row['SUM(total_tickets)'];
